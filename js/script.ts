let locationCard: string = `<div class="col mb-4">
<div class="card">
...
let restaurantCard: string = `<div class="col mb-4">
<div class="card">
  <img src="" class="card-img-top d-none d-md-block">
  <div class="card-body">
    <h5 class="card-title"></h5>
...
let eventCard: string = `<div class="col mb-4">
<div class="card">
...
let cards: any = [];
let calledSite: string = window.location.pathname.split("/")[window.location.pathname.split("/").length - 1];

function parseElement(html: string) {
  return new DOMParser().parseFromString(html, 'text/html').body.children[0];
}

class CLocation { //"Location" is already defined in ts
  title: string;
  description: string;
  city: string;
  zip: string;
  ...
  teaserImg: string;
  created: Date;
  constructor (title: string, description: string, city: string, zip: string, address: string, teaserImg: string, created: Date) {
    this.title = title;
  ...
  display(id: string) {
    let element: any  = parseElement(locationCard);
...
class CRestaurant extends CLocation {
  phone: string;
  website: string;
  constructor (title: string, description: string, city: string, zip: string, address: string, teaserImg: string, created: Date, phone: string, website: string) {
    super(title, description, city, zip, address, teaserImg, created);
    this.phone = phone;
    this.website = website;
  }
  display(id: string) {
    let element: any  = parseElement(restaurantCard);
    element.getElementsByClassName("card-img-top")[0].setAttribute("src", this.teaserImg);
    element.getElementsByClassName("card-title")[0].innerHTML = this.title;
    element.getElementsByClassName("card-text")[0].innerHTML = this.description;
    element.getElementsByClassName("list-group-item")[0].innerHTML = `${this.address}<br>${this.zip} ${this.city}`;
...
class CEvent extends CLocation { //"Event" is already defined in ts
  eventDate: string;
  eventTime: string;
  constructor (title: string, description: string, city: string, zip: string, address: string, teaserImg: string, created: Date, eventDate: string, eventTime: string) {
    super(title, description, city, zip, address, teaserImg, created);
    this.eventDate = eventDate;
    this.eventTime = eventTime;
  }
  display(id: string) {
    let element: any = parseElement(eventCard);
    element.getElementsByClassName("card-img-top")[0].setAttribute("src", this.teaserImg);
    element.getElementsByClassName("card-title")[0].innerHTML = this.title;
    element.getElementsByClassName("card-text")[0].innerHTML = this.description;
